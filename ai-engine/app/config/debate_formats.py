...
FORMAT_ALIASES: dict[str, str] = {
    "standard": "One-on-One Debate",
    "default": "One-on-One Debate",
    "one-on-one": "One-on-One Debate",
    "one on one": "One-on-One Debate",
    "one_on_one": "One-on-One Debate",
    "1v1": "One-on-One Debate",
    ...
    "ai simulation": "AI Debate Simulation",
    "ai debate simulation": "AI Debate Simulation",
    "simulation": "AI Debate Simulation",
}


def normalize_format_name(format_name: str) -> str:
    if not format_name:
        return "One-on-One Debate"
    cleaned = str(format_name).strip()
    if cleaned in DEBATE_FORMATS:
        return cleaned

    lower = cleaned.lower()
    if lower in FORMAT_ALIASES:
        return FORMAT_ALIASES[lower]

    for canonical in DEBATE_FORMATS:
...
def get_format_config(format_name: str) -> DebateFormatConfig:
    canonical = normalize_format_name(format_name)
    if canonical not in DEBATE_FORMATS:
        raise ValueError(
            f"Unknown debate format: '{format_name}'. Supported formats: {list(DEBATE_FORMATS.keys())}"
        )
    return DEBATE_FORMATS[canonical]
