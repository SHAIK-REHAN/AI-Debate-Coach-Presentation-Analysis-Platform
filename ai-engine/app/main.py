...
def extract_retry_after(error_str: str) -> int:
    match = re.search(r"Please retry in (\d+(\.\d+)?)s", str(error_str), re.IGNORECASE)
    if match:
        return max(1, int(float(match.group(1))))
    match_delay = re.search(r"retryDelay[\"']:\s*[\"'](\d+)s", str(error_str))
    if match_delay:
        return max(1, int(match_delay.group(1)))
    return 20


@app.exception_handler(GoogleRateLimitError)
async def google_rate_limit_exception_handler(request, exc: GoogleRateLimitError):
    raw_msg = str(exc)
    retry_sec = extract_retry_after(raw_msg)
    return JSONResponse(
        status_code=429,
        content={
            "error": "AI service rate limit reached",
            "retry_after": retry_sec,
            "message": f"Gemini quota is temporarily exhausted. Please try again in {retry_sec} seconds.",
            "detail": f"Gemini quota is temporarily exhausted. Please try again in {retry_sec} seconds."
        }
    )
...
try:
    ...
    async def google_client_error_handler(request, exc: ClientError):
        raw_msg = str(exc)
        if "RESOURCE_EXHAUSTED" in raw_msg or exc.code == 429 or "429" in raw_msg:
            retry_sec = extract_retry_after(raw_msg)
            return JSONResponse(
                status_code=429,
                content={
...
async def start_debate_session_stream(body: StartSessionRequest):
    ...
    return StreamingResponse(_phased_event_stream(gen, body.session_id, body.debate_format), media_type="text/event-stream", headers=SSE_HEADERS)
...
async def submit_debate_turn_stream(session_id: str, body: SubmitTurnRequest):
    ...
    return StreamingResponse(_phased_event_stream(gen, session_id, body.debate_format), media_type="text/event-stream", headers=SSE_HEADERS)
...
async def persist_turn_result(session_id: str, debate_format: str, result: dict, mode: str = "voice"):
    ...
    try:
        ...
        print(f"PostgreSQL debate_performance insert note: {e}")
...
async def process_typed_turn_stream(body: TypedTurnRequest):
    ...
    async def event_stream():
        try:
            ...
            raw_msg = str(e)
            if "429" in raw_msg or "RESOURCE_EXHAUSTED" in raw_msg:
                retry_sec = extract_retry_after(raw_msg)
                err_payload = {
                    "error": "AI service rate limit reached",
                    "retry_after": retry_sec,
...
async def admin_agent_performance():
    try:
        ...
        print(f"admin_agent_performance query note: {e}")
        return []
