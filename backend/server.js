...
app.get("/admin/agent-performance", verifyToken, requireRole("Admin"), async (req, res) => {
  try {
    const resp = await fetch(`${AI_ENGINE_URL}/api/v1/admin/agent-performance`, { signal: AbortSignal.timeout(5000) });
...
app.get("/admin/system-health", verifyToken, requireRole("Admin"), async (req, res) => {
  ...
  try {
    const resp = await fetch(`${AI_ENGINE_URL}/health`, { signal: AbortSignal.timeout(3000) });
...
app.get("/topics", verifyToken, async (req, res) => {
  try {
    ...
    for (const t of topics) {
      const key = t.title ? t.title.trim().toLowerCase() : String(t._id);
      if (!seen.has(key)) {
        seen.add(key);
        uniqueTopics.push(t);
      }
