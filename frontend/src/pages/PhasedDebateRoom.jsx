...
function PhasedDebateRoom() {
  ...
  const handleStart = async () => {
    ...
    try {
      const response = await fetch(`${AI_ENGINE_URL}/api/v1/debate/session/start-stream`, {
        method: "POST",
  ...
  const submitPhase = async (text, timedOut) => {
    ...
    setLoading(true); setError(""); setStreamingReply("");
    try {
      const response = await fetch(`${AI_ENGINE_URL}/api/v1/debate/session/${sessionId}/submit-stream`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const respondContinue = async (action) => {
    setLoading(true); setError(""); setStreamingReply("");
    try {
      const response = await fetch(`${AI_ENGINE_URL}/api/v1/debate/session/${sessionId}/continue-stream`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
            ...
            {transcript.map((t, i) => (
              <div key={i} className={`rounded-2xl p-4 border ${t.speaker === "user" ? "bg-purple-600/10 border-purple-500/30" : "bg-[#1a1a2b] border-white/5"}`}>
                ...
                {t.speaker === "user" ? (
                  ...
                  <FormattedText content={t.content || "(no response submitted)"} />
                )}
              </div>
            ))}
          </div>
              ...
              <div>
                ...
                {streamingReply && (
                  <div className="text-gray-300 text-sm leading-relaxed">
                    <FormattedText content={streamingReply} />
