...
function DebateRoom() {
  ...
  const consumeStream = async (response, onChunk, onDone) => {
    ...
    while (true) {
      ...
      for (const raw of events) {
        ...
        const dataStr = raw.slice(dataIndex + 5).trim();
        const data = JSON.parse(dataStr);
        if (eventType === "chunk") onChunk(data.text);
        else if (eventType === "done") onDone(data);
        else if (eventType === "error") {
          throw new Error(data.detail || data.message || "AI Engine stream error");
        }
      }
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(`${AI_ENGINE_URL}/api/v1/debate/turn-text-stream`, {
        method: "POST",
      ...
      console.error(error);
      alert(error.message || "Could not reach the AI engine. Make sure the Python service is running on localhost:8000.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleConfirmedTranscript = async (transcriptText, durationSec, audioBlob) => {
    ...
    try {
      ...
      const response = await fetch(`${AI_ENGINE_URL}/api/v1/debate/turn-stream`, {
        method: "POST",
        body: form
      });
      if (!response.ok) throw new Error(`Stream failed: ${response.status}`);
