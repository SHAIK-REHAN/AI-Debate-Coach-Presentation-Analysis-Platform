  ...
  return (
    <div className="mt-2 space-y-4">
      ...
      <div className="bg-[#0f0f1a] rounded-lg p-4 text-sm">
        ...
        <FormattedText content={result.ai_rebuttal} />
      </div>
      ...
      <div className="bg-[#0f0f1a] rounded-lg p-4 text-sm">
        ...
        {result.fallacy_metrics.fallacy_detected ? (
          <div className="space-y-3">
            {(result.fallacy_metrics.fallacies && result.fallacy_metrics.fallacies.length > 0
              ? result.fallacy_metrics.fallacies
              : [result.fallacy_metrics]
            ).map((f, i) => (
              <div key={i} className="border-b border-white/5 pb-2.5 last:border-0 last:pb-0">
                <p className="text-red-400 font-medium mb-1">{f.fallacy_type} detected</p>
                {f.offending_text && <p className="text-gray-400 mb-1 italic">"{f.offending_text}"</p>}
                {f.explanation && <p className="text-gray-400 mb-1">{f.explanation}</p>}
                {f.correction_suggestion && <p className="text-purple-300 text-xs mt-0.5">{f.correction_suggestion}</p>}
              </div>
            ))}
          </div>
        ) : (
